        ...
        <div class="col-lg-8">
            <div class="card">
                <div class="card-header bg-primary text-white">
                    <h5 class="mb-0">Create New Target</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('targets.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="user_id" id="user_id" class="form-select @error('user_id') is-invalid @enderror">
                                ...
                                @foreach($employees as $employee)
                                    <option value="{{ $employee->UserID }}" {{ old('user_id') == $employee->UserID ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="target_type" id="target_type" class="form-select @error('target_type') is-invalid @enderror" required>
                                ...
                                <option value="day" {{ old('target_type') == 'day' ? 'selected' : '' }}>Day Target</option>
                                <option value="month" {{ old('target_type') == 'month' ? 'selected' : '' }}>Month Target</option>
                            </select>
                            @error('target_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="target_amount" id="target_amount" class="form-control @error('target_amount') is-invalid @enderror"
                                   placeholder="0.00" step="0.01" value="{{ old('target_amount') }}" required>
                            @error('target_amount')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror"
                                      rows="3" placeholder="Any notes about this target">{{ old('description') }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="d-flex gap-2">
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-check"></i> Create Target
                            </button>
