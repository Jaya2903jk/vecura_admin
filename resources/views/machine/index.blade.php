        ...
        <div class="content px-4 py-3">
            ...
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4 pb-3 border-bottom">
                <div>
                    <h4 class="fw-bold text-dark mb-1 d-flex align-items-center gap-2">
                        <i class="ti ti-cpu text-primary fs-24"></i>Machine Master
                        <span class="badge bg-primary-subtle text-primary border border-primary-subtle fs-12 ms-2 px-2.5 py-1 rounded-pill">
                            Total Machines: {{ $Machines->total() }}
                        </span>
                    </h4>
                    <p class="text-muted fs-13 mb-0">Manage registered machines, equipment details, and operational statuses.</p>
                </div>
                <div class="d-flex align-items-center gap-2">
                    @if(session('is_admin') || \App\Helpers\RbacHelper::canPerformAction('create', 'machine'))
                        <button type="button" class="btn btn-primary btn-sm fw-bold px-3 shadow-xs" data-bs-toggle="modal" data-bs-target="#add_modal">
                            <i class="ti ti-plus me-1"></i>New Machine Registration
                        </button>
                    @endif
                </div>
            </div>
            ...
            <div class="card border-0 shadow-xs mb-4 rounded-3">
                <div class="card-body p-3 bg-light-subtle rounded-3">
                    <form method="GET" action="{{ route('machine.index') }}" id="machineFilterForm">
                        <div class="row g-2 align-items-center">
                            {{-- Search Input --}}
                            <div class="col-md-6">
                                <label class="form-label fw-semibold fs-13 text-dark mb-1">Search Machine</label>
                                <div class="input-group input-group-sm">
                                    <span class="input-group-text bg-white border-end-0"><i class="ti ti-search text-muted"></i></span>
                                    <input type="text" name="search" class="form-control border-start-0 fs-13 text-dark"
                                        placeholder="Machine Name..." value="{{ request('search') }}">
                                </div>
                            </div>
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="status" class="form-select form-select-sm fs-13 text-dark" onchange="document.getElementById('machineFilterForm').submit()">
                            ...
                            <div class="col-md-3 d-flex align-items-end gap-1 pt-3">
                                ...
                                <a href="{{ route('machine.index') }}" class="btn btn-light border btn-sm fw-semibold fs-13 text-secondary" title="Reset Filters">
                    ...
                    <div class="table-responsive" style="overflow-x: auto;">
                        <table class="table table-hover align-middle mb-0">
                            <thead class="bg-light border-bottom">
                                <tr>
                                    <th class="fs-12 text-uppercase fw-bold text-slate-700 py-3 ps-4">Machine Name</th>
                                        ...
                                        <td class="ps-4">
                                            <div class="d-flex align-items-center gap-2">
                                                <div class="patient-avatar-circle flex-shrink-0 text-decoration-none">
                                                    <i class="ti ti-cpu fs-16 text-primary"></i>
                                                </div>
                                                <div>
                                                    <span class="fw-bold text-dark fs-13">{{ $machine->MachineName }}</span>
                                                    <div class="fs-11 text-muted">ID: #{{ $machine->MachineId }}</div>
                                                </div>
                                            </div>
                                        </td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="3" class="text-center text-muted py-5 fs-13">
                                            <i class="ti ti-cpu-off fs-36 text-muted mb-2 d-block"></i>
            ...
            <div class="table-footer-bar d-flex flex-column flex-md-row justify-content-between align-items-center mt-3 fs-13 bg-white p-3 rounded-3 shadow-xs border gap-3">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div class="text-muted border-start ps-3">
                        Showing <span class="fw-semibold text-dark">{{ $Machines->firstItem() ?? 0 }}</span> to <span class="fw-semibold text-dark">{{ $Machines->lastItem() ?? 0 }}</span> of <span class="fw-semibold text-dark">{{ $Machines->total() }}</span> entries
                    </div>
                </div>
                <div>
                    <x-pagination :paginator="$Machines" :append="['per_page' => $perPage, 'search' => request('search'), 'status' => request('status')]" />
                </div>
            </div>
            ...
            <div class="modal-content border-0 shadow">
                <div class="modal-header border-bottom px-4 py-3 bg-light">
                    <h5 class="modal-title fw-bold text-dark d-flex align-items-center gap-2">
                        <i class="ti ti-cpu text-primary fs-20"></i>Add New Machine
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>

                <form id="machineForm" class="needs-validation" novalidate>
                    @csrf
                    <div class="modal-body p-4">
                        <div class="mb-3">
                            <label class="form-label fw-semibold fs-13 text-dark">Machine Name <span class="text-danger">*</span></label>
                            <input type="text" name="machine_name" id="machine_name" class="form-control fs-13"
                    ...
                    <div class="modal-footer px-4 py-3 bg-light border-top gap-2">
                        ...
                        <button type="submit" id="submitBtn" class="btn btn-primary btn-sm fw-bold px-3">
                            <i class="ti ti-plus me-1"></i>Add New Machine
                        </button>
                    </div>
                </form>
            </div>
            ...
            $('#machineForm').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        submitBtn.prop('disabled', false).html('<i class="ti ti-plus me-1"></i>Add New Machine');
