            ...
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4 pb-3 border-bottom">
                <div>
                    <h4 class="fw-bold text-dark mb-1 d-flex align-items-center gap-2">
                        <i class="ti ti-map text-primary fs-24"></i>State Master
                    ...
                    <p class="text-muted fs-13 mb-0">Manage state jurisdictions, codes, and country mappings.</p>
                </div>
                <div class="d-flex align-items-center gap-2">
                    <button type="button" class="btn btn-primary btn-sm fw-bold px-3 shadow-xs" data-bs-toggle="modal" data-bs-target="#add_modal">
                        <i class="ti ti-plus me-1"></i>New State Registration
                    </button>
                </div>
            </div>
                    ...
                    <div class="table-responsive" style="overflow-x: auto;">
                        <table class="table table-hover align-middle mb-0">
                            <thead class="bg-light border-bottom">
                                <tr>
                                    <th class="fs-12 text-uppercase fw-bold text-slate-700 py-3 ps-4">State Name</th>
                                    <th class="fs-12 text-uppercase fw-bold text-slate-700 py-3">State Code</th>
                                    ...
                                    <tr>
                                        <td class="ps-4">
                                            <div class="d-flex align-items-center gap-2">
                                                <div class="patient-avatar-circle flex-shrink-0 text-decoration-none">
                                                    <i class="ti ti-map text-primary fs-16"></i>
                                                </div>
                                                <div>
                                                    <span class="fw-bold text-dark fs-13">{{ $state->state_name }}</span>
                                                    <div class="fs-11 text-muted">ID: #{{ $state->state_id }}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <span class="patient-code-tag font-monospace">{{ $state->state_code }}</span>
                                        </td>
                                        <td>
                                            <div class="fs-13 text-dark fw-semibold">{{ $state->country?->country_name ?? '-' }}</div>
                                        </td>
                                        <td>
                                            @if ($state->is_active)
                                        ...
                                        <td class="text-end pe-4">
                                            <div class="d-flex align-items-center justify-content-end gap-1">
                                                <button type="button" class="btn btn-light border btn-sm px-2 py-1 shadow-2xs edit-btn" title="Edit State"
                                                    data-bs-toggle="modal" data-bs-target="#edit_modal"
                                                    data-id="{{ $state->state_id }}"
                                                    data-code="{{ $state->state_code }}"
                                                    data-name="{{ $state->state_name }}"
                                                    data-country="{{ $state->country_id }}"
                                                    data-active="{{ $state->is_active ? 1 : 0 }}">
                                                    <i class="ti ti-edit fs-15 text-warning"></i>
                                                </button>
                                                <button type="button" class="btn btn-light border btn-sm px-2 py-1 shadow-2xs delete-btn" title="Delete State"
                                                    data-id="{{ $state->state_id }}"
                                                    data-name="{{ $state->state_name }}"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center text-muted py-5 fs-13">
                                            ...
                                            No states found matching criteria.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
            ...
            <div class="table-footer-bar d-flex flex-column flex-md-row justify-content-between align-items-center mt-3 fs-13 bg-white p-3 rounded-3 shadow-xs border gap-3">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div class="text-muted border-start ps-3">
                        Showing <span class="fw-semibold text-dark">{{ $states->firstItem() ?? 0 }}</span> to <span class="fw-semibold text-dark">{{ $states->lastItem() ?? 0 }}</span> of <span class="fw-semibold text-dark">{{ $states->total() }}</span> entries
                    </div>
                </div>
                <div>
                    <x-pagination :paginator="$states" :append="['per_page' => $perPage]" />
                </div>
            </div>
            ...
            <div class="modal-content border-0 shadow">
                <div class="modal-header border-bottom px-4 py-3 bg-light">
                    <h5 class="modal-title fw-bold text-dark d-flex align-items-center gap-2">
                        <i class="ti ti-map text-primary fs-20"></i>Add New State
                    </h5>
                ...
                <form id="addForm">
                    ...
                    <div class="modal-body p-4">
                        <div class="row g-3">
                            <div class="col-md-4">
                                <label class="form-label fw-semibold fs-13 text-dark">State Code <span class="text-danger">*</span></label>
                                <input type="text" name="state_code" class="form-control fs-13 text-uppercase" maxlength="10" placeholder="e.g. TN" required>
                            </div>
                            <div class="col-md-8">
                                <label class="form-label fw-semibold fs-13 text-dark">State Name <span class="text-danger">*</span></label>
                                <input type="text" name="state_name" class="form-control fs-13" placeholder="Enter state name" required>
                            </div>
                            <div class="col-md-12">
    ...
    <div id="edit_modal" class="modal fade" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow">
                <div class="modal-header border-bottom px-4 py-3 bg-light">
                    <h5 class="modal-title fw-bold text-dark d-flex align-items-center gap-2">
                        <i class="ti ti-edit text-warning fs-20"></i>Edit State
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form id="editForm">
                    ...
                    <div class="modal-body p-4">
                        <div class="row g-3">
                            <div class="col-md-4">
                                <label class="form-label fw-semibold fs-13 text-dark">State Code <span class="text-danger">*</span></label>
                                <input type="text" name="state_code" id="edit_state_code" class="form-control fs-13 text-uppercase" maxlength="10" required>
                            </div>
                            <div class="col-md-8">
                                <label class="form-label fw-semibold fs-13 text-dark">State Name <span class="text-danger">*</span></label>
                                <input type="text" name="state_name" id="edit_state_name" class="form-control fs-13" required>
                            </div>
                            <div class="col-md-12">
                                <label class="form-label fw-semibold fs-13 text-dark">Country <span class="text-danger">*</span></label>
        ...
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content border-0 shadow">
                <div class="modal-body text-center p-4">
                    ...
                    <h5 class="fw-bold text-dark mb-1">Delete State</h5>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.edit-btn', function() {
                ...
                $('#edit_state_code').val($(this).data('code'));
            ...
            $('#addForm').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        btn.prop('disabled', false).html('<i class="ti ti-plus me-1"></i>Add State');
                        if (res.status) {
